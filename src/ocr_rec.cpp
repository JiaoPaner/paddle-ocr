...
#include "cJSON.h"
namespace PaddleOCR {
    char* CRNNRecognizer::Run(std::vector<std::vector<std::vector<int>>> boxes,cv::Mat &img, Classifier *cls) {
        cv::Mat image;
        img.copyTo(image);
        cv::Mat crop_img;
        cv::Mat resize_img;
        ...
        for (int i = boxes.size() - 1; i >= 0; i--) {
            crop_img = GetRotateCropImage(image, boxes[i]);
            if (cls != nullptr) {
            ...
            for (int n = 0; n < pred_idx.size(); n++)
                text.push_back(label_list_[pred_idx[n]]);
            ...
            std::string value;
            cJSON_AddStringToObject(item, "text", accumulate(text.begin(),text.end(),value).c_str());
            ...
            cJSON_AddNumberToObject(location, "x", left);
            cJSON_AddNumberToObject(location, "y", right);
            cJSON_AddNumberToObject(location, "width", right - left);
            cJSON_AddNumberToObject(location, "height", bottom - top);
    ...
    cv::Mat CRNNRecognizer::GetRotateCropImage(const cv::Mat &srcimage,std::vector<std::vector<int>> box) {
